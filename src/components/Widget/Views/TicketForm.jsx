...
function TicketForm({ onNavigate, params = {} }) {
  ...
  useEffect(() => {
    ...
    // Инициализация глобальных полей
    ...
    // AI-префиллинг из параметров
    if (params.initialData) {
      const aiData = params.initialData
      
      // Установка категории от ИИ
      if (aiData.category && categories[aiData.category]) {
        initialCategory = aiData.category
        setAiSelectedCategory(aiData.category)
      }

      // Заполнение полей от ИИ
      ...
      // Стандартное предзаполнение из params
      if (params.query) {
  ...
  useEffect(() => {
    ...
    // Сохраняем глобальные поля
    const globalFieldNames = globalFields.map(f => f.name)
    const preservedData = {}
    ...
    // Добавляем поля новой категории
    newCategoryFields.forEach(field => {
    ...
    // Удаляем AI-индикацию для полей, которых больше нет
    setAiFilledFields(prev => {
      const newSet = new Set(prev)
      const currentFieldNames = [
        ...globalFieldNames,
        ...newCategoryFields.map(f => f.name)
      ]
  ...
  const handleCategoryChange = (newCategory) => {
    ...
    setAiSelectedCategory(null) // Сбрасываем AI-категорию при ручном изменении
  }
  ...
  const handleInputChange = (name, value) => {
    ...
    // Удаляем AI-индикацию при ручном редактировании
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      ...
      console.log('FinalTicketData:', ticketData)
  ...
  const isFormValid = () => {
    // Проверка глобальных полей
    const globalValid = globalFields.every(field => {
    ...
    const categoryFields = categories[selectedCategory]?.fields || []
    const categoryValid = categoryFields.every(field => {
  ...
  return (
    <div className="p-3 md:p-4">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Предупреждение при смене категории */}
        {showCategoryWarning && (
