...
function ArticleView({ article, onNavigate, params = {} }) {
  const [isTransitioning, setIsTransitioning] = useState(false)
  const contentRef = useRef(null)

  useEffect(() => {
    // Скролл вверх при смене статьи
    if (contentRef.current) {
      // Находим родительский scrollable контейнер
  ...
  // Получаем связанные статьи (исключая текущую)
  ...
  const handleFeedback = (helped) => {
    if (helped) {
      // Если был поиск, возвращаемся к результатам, иначе на главную
      ...
      // Сначала предлагаем ИИ, потом тикет
