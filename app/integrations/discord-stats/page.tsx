...
import DiscordStatsConnectionTab from '../../../components/admin/IntegrationSettings/DiscordStatsConnectionTab'
import DiscordStatsGlobalLimitsTab from '../../../components/admin/IntegrationSettings/DiscordStatsGlobalLimitsTab'
import { getIntegrationById, Integration, IntegrationSettings } from '../../../data/integrations'

export default function DiscordStatsIntegrationPage() {
  ...
  return (
    <div className="flex h-screen bg-[#f8fafc]">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <Header onMenuToggle={() => setSidebarOpen(!sidebarOpen)} />
        <main className="flex-1 overflow-y-auto">
          <DiscordStatsIntegrationContent integration={integration} />
        </main>
      </div>
    </div>
  )
...
function DiscordStatsIntegrationContent({ integration }: { integration: Integration }) {
  ...
  const tabs = [
    ...
    { id: 'global_limits', label: 'Global Limits' },
  ]

  const handleSave = () => {
    // TODO: Implement save logic
    console.log('Saving Discord Stats integration:', { ...integration, settings, isActive })
