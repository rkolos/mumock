...
export default function TicketsList() {
  ...
  const getFilteredTickets = (): Ticket[] => {
    ...
    // Smart View filters (применяются первыми)
    if (activeView) {
      switch (activeView) {
        case 'your_inbox':
          filtered = filtered.filter(t => 
            t.assignedUsers.includes(CURRENT_USER_EMAIL) && t.status !== 'closed'
          )
          break
        case 'mentions':
          filtered = filtered.filter(t => 
            t.mentions?.includes(CURRENT_USER_EMAIL)
          )
          break
        case 'awaiting_reply':
          filtered = filtered.filter(t => 
            t.assignedUsers.includes(CURRENT_USER_EMAIL) && t.lastMessageAuthorId === 'user'
          )
          break
        case 'unassigned':
          filtered = filtered.filter(t => 
            t.assignedUsers.length === 0 && t.status !== 'closed'
          )
          break
        case 'all_tickets':
          // Без дополнительной фильтрации (показать все)
          break
      }
    }
  ...
  useEffect(() => {
    ...
    // View
    if (viewParam) {
      const validViews = ['your_inbox', 'mentions', 'awaiting_reply', 'unassigned', 'all_tickets']
      if (validViews.includes(viewParam) && activeView !== viewParam) {
  ...
  }, [activeView, sourceFilter, statusFilters, priorityFilters, assignedUsersFilters, tagsFilters, categoriesFilters, dateFilter, waitingTimeFilter, router])
