...
interface DiscordCategoriesTabProps {
  integrationId: string
}

export default function DiscordCategoriesTab({ integrationId }: DiscordCategoriesTabProps) {
  const router = useRouter()
  const [categories, setCategories] = useState<Category[]>(
    mockCategories.filter((cat) => cat.source_type === 'discord')
  )
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage categories that use Discord Bot integration
          </p>
        </div>
