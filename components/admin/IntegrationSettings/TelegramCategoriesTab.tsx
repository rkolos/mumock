...
interface TelegramCategoriesTabProps {
  integrationId: string
}

export default function TelegramCategoriesTab({ integrationId }: TelegramCategoriesTabProps) {
  const [categories, setCategories] = useState<Category[]>(
    mockCategories.filter((cat) => cat.source_type === 'telegram')
  )
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Manage categories that use Telegram Bot integration
          </p>
        </div>
        <button
